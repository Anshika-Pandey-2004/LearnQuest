...
exports.deleteCourse = async (req, res) => {
  try {
    ...
    // 2. Unenroll all students (remove course reference)
    await User.updateMany(
      { _id: { $in: course.studentsEnrolled } },
      { $pull: { courses: courseId } }
    );

    // 3. Remove course from instructor
    await User.findByIdAndUpdate(course.instructor, {
      $pull: { courses: courseId },
    });

    // ⭐ 4. Remove courseProgress refs from user
    const progressDocs = await CourseProgress.find({ courseID: courseId });

    if (progressDocs.length > 0) {
      const progressIds = progressDocs.map((p) => p._id);

      // Remove from user.courseProgress[]
      await User.updateMany(
    ...
    // ⭐ 5. Delete all courseProgress docs for this course
    await CourseProgress.deleteMany({ courseID: courseId });

    // 6. Delete all sections + subsections
    ...
    // 7. Delete Ratings & Reviews
    await RatingAndReview.deleteMany({ course: courseId });

    // ⭐ 8. Remove course from Category model (IMPORTANT FIX)
    if (course.category) {
      await Category.findByIdAndUpdate(course.category, {
        $pull: { course: courseId }, // correct field name
      });
    }

    // 9. Delete final Course document
